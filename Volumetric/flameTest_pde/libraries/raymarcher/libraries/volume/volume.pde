...
class Cloud implements Volume
{
  vRes  sample( PVector p )
  {
    ...
    float d = p.mag();
     float nf = 1.f;
     float namp = 3.f;
    d += (noise(p.x*nf+57.f,p.y*nf+513.f,p.z*nf+13.f)-.5)*namp;
...
class Nebula2 implements Volume
{
  vRes  sample( PVector p )
  {
    ...
    d *=2.;
     float nf = 1.f;
     float namp = 6.f;
    d += (abs(noise(p.x*nf+357.f,p.y*nf+53.f,p.z*nf+13.f)-.5)-.25)*namp;
    d = max(d,0.);
...
class HieghtField implements Volume
{
  ...
  vRes  sample( PVector p )
  {
    ...
    float nf = 1.f;
    noiseDetail(5,0.6f);
    d += (abs(noise(p.x*nf+57.f,p.z*nf+13.f)-.5)-.25)*1.f;
...
class ColouredSphere implements Volume
{
  ...
  vRes  sample( PVector p )
  {
    float d = p.mag();
    float nf = 3.f;
    d += (abs(noise(p.x*nf+57.f,p.y*nf+513.f,p.z*nf+13.f)-.5)-.25)*1.f;
